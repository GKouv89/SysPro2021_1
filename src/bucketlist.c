...
void popStatus_all(bucketList *bl, int ageMode, Virus *v, char *startingDate, char *endingDate){
  if(bl->type == Country_List){
    bucketNode *temp = bl->front;
    while(temp){
      void *vacced = (struct vaccinations *) skiplist_vac_status_country(v->vaccinated_for, 1, ageMode, (Country *)temp->content, startingDate, endingDate);
      void *notVacced = (struct vaccinations *) skiplist_vac_status_country(v->not_vaccinated_for, 0, ageMode, (Country *)temp->content, startingDate, endingDate);
      if(ageMode == 0){
